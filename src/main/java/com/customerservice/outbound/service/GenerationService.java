...
public class GenerationService {
	...
	public List<File> processAll(final FormData data) throws ConvertingException {
		...
		try {
			...
			FutureTask<File> letter = new FutureTask<>(new Callable<File>() {
				...
				public File call() {
					try {
						return generateLetter(data, letterName);
					} catch (Exception e) {
						return null;
					}
				}
			...
			logger.debug("started thred for letter");
			...
			logger.debug("started threads for uploads");
			...
			logger.info("processed all data and persist the output");
	 ...
	 */
	public File generateLetter(FormData data, String fileName) throws IOException, DocumentException, NoSuchAlgorithmException {
		...
		data.setMessage(data.getMessage().replace("image?id=", image)); //replace dynamic request url with local file path
	 ...
	 * @throws NoSuchAlgorithmException
	 */
	public File mergeDir(String id, String fileName) throws IOException, DocumentException, NoSuchAlgorithmException {
		...
		for (String name : fileNames) {
			...
			files.put(name, new PdfReader(output + dirName + "/" + name)); //if we have a non pdf file present -> exception
		}
